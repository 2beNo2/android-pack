...
JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    //动态注册
    jclass clsMyApp =  env->FindClass("org/test/androidpack3/MyApp");
    if (env->ExceptionCheck()) {
        env->ExceptionDescribe();
        env->ExceptionClear();
        LOGE("[%s] FindClass Error name:%s", __FUNCTION__, "org/test/androidpack3/MyApp");
        return result;
    }
